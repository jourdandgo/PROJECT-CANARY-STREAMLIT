...
def train_model(X, y):
    ...
    # Calculate defensible metrics
    y_pred = rf.predict(X_test)
    y_prob = rf.predict_proba(X_test)[:, 1]
    
    # If the test set has only one class (extreme imbalance), ROC AUC will fail
    try:
...
model, model_acc, model_auc = train_model(X, y)
...
with st.sidebar:
    ...
    base_cost = st.slider("Base Cost / Chick", 50, 200, st.session_state.base_cost, key="base_cost")
    growth_cost = st.slider("Daily Growth Cost", 1.0, 20.0, st.session_state.growth_cost, key="growth_cost")
...
with h2:
    ...
    with inner1:
        ...
        feature_labels = {
            'Max_Temperature_C': "High Temperature",
            'Avg_Humidity_Percent': "Humidity Imbalance",
            'Avg_Water_Intake_ml': "Low Water Intake",
            'Avg_Feed_Intake_g': "Low Feed Intake",
            'Bird_Age_Days': "Bird Age Stress",
            'THI': "Heat-Humidity Index",
            'Water_to_Feed_Ratio': "Water/Feed Imbalance",
            'Feed_Intake_Delta': "Sudden Feed Drop"
        }
...
with lc1:
    ...
    sim_t = st.slider("Max Temperature (°C)", t_min, 45.0, c_temp, key=f"sim_temp_{st.session_state.active_zone}")
    ...
    sim_h = st.slider("Humidity (%)", h_min, 100.0, c_hum, key=f"sim_hum_{st.session_state.active_zone}")
    ...
    sim_w = st.slider("Water Intake (ml)", w_min, 1000.0, c_water, key=f"sim_water_{st.session_state.active_zone}")
    ...
    sim_f = st.slider("Feed Intake (g)", f_min, 500.0, c_feed, key=f"sim_feed_{st.session_state.active_zone}")
